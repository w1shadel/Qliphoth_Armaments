...
public class PlayerChainRenderer extends EntityRenderer<PlayerChainEntity> {
    ...
    @Override
    public boolean shouldRender(PlayerChainEntity pLivingEntity, net.minecraft.client.renderer.culling.Frustum pCamera,
                                double pCamX, double pCamY, double pCamZ) {
        return true;
    }

    @Override
    public void render(PlayerChainEntity chain, float yaw, float pticks, PoseStack matrices, MultiBufferSource src,
                       int light) {
        Vec3 startPos;
        ...
        if (length < 0.01f)
            return;
        matrices.pushPose();
        org.joml.Vector3f to = new org.joml.Vector3f((float) between.x, (float) between.y, (float) between.z)
                .normalize();
    ...
    private void drawChainSegmentAnimated(VertexConsumer b, Matrix4f m, float length, float width, int light,
                                          float vScale, float vOffset) {
