...
public class PlayerChainEntity extends LivingEntity {
    private static final EntityDataAccessor<Integer> ATTACK_TYPE_ORDINAL = SynchedEntityData
            .defineId(PlayerChainEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> OWNER_ID = SynchedEntityData.defineId(PlayerChainEntity.class,
            EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> IS_ATTACK_MODE = SynchedEntityData
            .defineId(PlayerChainEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Vector3f> ORIGIN_POS = SynchedEntityData.defineId(PlayerChainEntity.class,
            EntityDataSerializers.VECTOR3);
    private static final EntityDataAccessor<Boolean> ADD_SIN = SynchedEntityData.defineId(PlayerChainEntity.class,
    ...
    public static void summon(Level level, Player owner, QAElements type, LivingEntity target, int pullTime,
                              int catchTime) {
    ...
    public static void summonAttack(Level level, Player owner, QAElements type, LivingEntity target, Vec3 portalPos,
                                    float damage, boolean addSin) {
        PlayerChainEntity chain = new PlayerChainEntity(ModEntities.PLAYER_CHANE.get(), level);
        chain.setupCommon(owner, target, type);
    ...
    private void tickAttack() {
        ...
        if (this.tickCount >= this.catchTime && !hasHit) {
    ...
    public void tickPull() {
        ...
        if (owner == null || !owner.isAlive() || target == null || !target.isAlive()
                || this.tickCount > this.catchTime + this.pullTime + 20) {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(ORIGIN_POS, new Vector3f(Float.NaN, Float.NaN, Float.NaN));
        this.entityData.define(IS_ATTACK_MODE, false);
        this.entityData.define(ADD_SIN, false);
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("PullToX")) {
            this.chainPullToPos = new Vec3(tag.getDouble("PullToX"), tag.getDouble("PullToY"),
                    tag.getDouble("PullToZ"));
        }
